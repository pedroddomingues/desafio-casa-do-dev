...
let TransactionsService = class TransactionsService {
    ...
    async deposit(createTransactionDto) {
        let account = await this.AccountsService.findOneByCPF(createTransactionDto.cpf);
        if (!account)
    ...
    async withdrawal(createTransactionDto, req) {
        ...
        console.log(user);
        await this.AuthService.checkPassword(createTransactionDto.password, user);
        ...
        const newBalance = this.newBalance(account, value, type, account._id.toString());
        ...
        return {
            ...
            overdraftLimitLeft: account.balance < 0
    ...
    async transfer(createTransactionDto, req) {
        ...
        const accountTo = await this.AccountsService.findOneByCPF(createTransactionDto.cpf);
        if (!accountTo)
            throw new common_1.HttpException({
                message: "Account not found",
            }, common_1.HttpStatus.BAD_REQUEST);
        ...
        const newBalanceFrom = this.newBalance(account, value, type, account._id.toString());
        ...
        await account.save();
        const newBalanceTo = this.newBalance(accountTo, value, type, account._id.toString());
