...
let AccountsService = class AccountsService {
    constructor(accountModel, SequentialKeysService, UsersService, TransactionsService) {
    ...
    async create(createAccountDto) {
        ...
        let sequential_id_number = await this.SequentialKeysService.getSequentialKey("account");
        if (sequential_id_number === 0) {
            return null;
        }
        const sequential_id = this.SequentialKeysService.sequentialKeyToZerosString(sequential_id_number, 6);
    ...
    async getBalance(req) {
        const user = await (await this.UsersService.findOneByID(req.user.sub)).populate("account");
        return {
            balance: user.account.balance,
            overdraftLimitLeft: user.account.balance < 0
    ...
    async getStatement(req, paginationQuery) {
        ...
        const transactions = await this.TransactionsService.getTransactionsByDate(user.account, startDate, endDate);
